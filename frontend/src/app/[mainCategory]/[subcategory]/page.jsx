...
export default function SubcategoryPage() {
    ...
    useEffect(() => {
        const fetchCategoryData = async () => {
            try {
                ...
                const mainCategoryResponse = await api.getMainCategories();
                const mainCategoryData = Array.isArray(mainCategoryResponse)
                    ? mainCategoryResponse.find(cat => cat.slug === mainCategory)
                    : mainCategoryResponse?.data?.find(cat => cat.slug === mainCategory);
                setMainCategoryData(mainCategoryData);

                // Fetch subcategory details
                const categoryData = await api.getCategoryBySlug(subcategory);
                setCategory(categoryData);

                // Fetch filter options for this category
                const filterData = await api.getCategoryFilterOptions(subcategory);
                ...
                const productsData = await api.getSubcategoryProducts(subcategory, mainCategory, {
                    sort: sortBy,
                    ...filters
                });
                ...
                setCategory({
                    ...
                    description: `Explore our amazing collection of ${subcategory?.replace('-', ' ')} products`,
                    image: '/images/category-placeholder.jpg'
                });
                setMainCategoryData({
                    name: mainCategory?.replace('-', ' '),
                    slug: mainCategory
                });
                setProducts([]);
                setFilterOptions({
    ...
    }, [sortBy, filters, category, loading, mainCategory, subcategory]);
    ...
    return (
        <>
            <Metadata
                ...
                canonicalUrl={`${process.env.NEXT_PUBLIC_SITE_URL || 'http://localhost:3000'}/${mainCategory}/${subcategory}`}
            />
            <div className="min-h-screen bg-gray-50">
                    ...
                    <div className="relative z-10 container-custom h-full flex flex-col justify-center">
                        <div className="text-center">
                            ...
                            <nav className="flex items-center justify-center space-x-2 text-sm text-white/80 mb-6">
                                ...
                                <ChevronLeft className="w-4 h-4" />
                                <Link href={`/collections/${mainCategory}`} className="hover:text-white transition-colors capitalize">
                                    {mainCategoryData?.name || mainCategory?.replace('-', ' ')}
                                </Link>
                                <ChevronLeft className="w-4 h-4" />
                            ...
                            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-4 mb-6">
                                <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
                                    ...
                                    <div className="flex items-center gap-3">
                                        <select
                                            className="border border-gray-200 text-gray-900 cursor-pointer rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-transparent transition-all duration-200"
                                            value={sortBy}
                                        ...
                                        <div className="flex border border-gray-200 rounded-lg overflow-hidden">
                                            <button
                                                className={`p-2 transition-all duration-200 cursor-pointer ${viewMode === 'grid'
                                            ...
                                            <button
                                                className={`p-2 transition-all duration-200 cursor-pointer ${viewMode === 'list'
                                                    ? 'bg-primary-600 text-white'
                                                    : 'bg-white text-gray-600 hover:bg-gray-50'
                                                    }`}
