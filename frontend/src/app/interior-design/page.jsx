    ...
    <div className="min-h-screen bg-white">
        ...
        <div className="container-custom py-16 sm:py-24">
          <div className="grid grid-cols-1 lg:grid-cols-5 gap-10 lg:gap-16 items-center">
            ...
            <div className="lg:col-span-3">
              <span className="inline-flex items-center gap-1.5 bg-red-600/20 border border-red-500/30 text-red-300 text-xs font-bold uppercase tracking-widest px-3 py-1.5 rounded-full mb-6">
            ...
            <div className="lg:col-span-2 grid grid-cols-2 gap-3">
              {services.map(({ icon: Icon, title, subtitle, href }) => (
                <Link
                  key={href}
                  href={href}
                  className="group bg-white/5 hover:bg-white/10 border border-white/10 hover:border-red-500/40 rounded-2xl p-5 flex flex-col gap-3 transition-all duration-200"
                >
                  <div className="flex items-start justify-between">
                    <div className="w-9 h-9 bg-red-600/20 group-hover:bg-red-600 rounded-lg flex items-center justify-center transition-colors duration-200">
                      <Icon className="w-4.5 h-4.5 text-red-300 group-hover:text-white transition-colors" />
                    </div>
                    <ArrowUpRight className="w-4 h-4 text-gray-600 group-hover:text-red-400 transition-colors" />
                  </div>
                  <div>
      ...
      <div className="bg-red-600">
        <div className="container-custom py-6">
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 sm:gap-0 sm:divide-x divide-red-500">
            {stats.map(({ num, label }) => (
              <div key={label} className="text-center sm:px-6">
                <div className="text-2xl sm:text-3xl font-extrabold text-white">{num}</div>
                <div className="text-xs text-red-200 font-medium mt-0.5">{label}</div>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="container-custom py-3 flex flex-wrap items-center justify-center gap-x-8 gap-y-2">
          {['Free Consultation', 'Premium Materials', 'On-Time Delivery', 'Expert Designers', '5-Year Warranty'].map((f) => (
            <span key={f} className="flex items-center gap-1.5 text-xs text-gray-600 font-medium">
              <CheckCircle className="w-3.5 h-3.5 text-red-500 flex-shrink-0" /> {f}
            </span>
          ))}
        </div>
      </div>
        ...
        <div className="container-custom">
          ...
          <div className="divide-y divide-gray-100">
            {services.map(({ num, icon: Icon, title, subtitle, description, features, href }) => (
              ...
              >
                {/* Big faded number */}
                <span className="text-5xl sm:text-7xl font-extrabold text-gray-100 group-hover:text-red-100 leading-none flex-shrink-0 select-none transition-colors pt-1 hidden sm:block">
                  {num}
                </span>
                  ...
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex-1">
                      <div className="flex items-center gap-2 mb-2">
                        <div className="w-8 h-8 bg-red-50 group-hover:bg-red-600 rounded-lg flex items-center justify-center transition-colors duration-300 flex-shrink-0">
                          <Icon className="w-4 h-4 text-red-600 group-hover:text-white transition-colors duration-300" />
                        </div>
                        <span className="text-[11px] font-bold uppercase tracking-widest text-red-500">{subtitle}</span>
                      </div>
                      <h3 className="text-xl sm:text-2xl font-extrabold text-gray-900 mb-2 group-hover:text-red-700 transition-colors">
                        {title}
                      </h3>
                    ...
                    {/* Arrow button */}
                    <div className="w-10 h-10 bg-gray-100 group-hover:bg-red-600 rounded-full flex items-center justify-center flex-shrink-0 transition-colors duration-300 mt-1">
      ...
      <div className="bg-gray-50 py-16 sm:py-20 border-t border-gray-100">
        <div className="container-custom">
          <div className="text-center mb-12">
            <span className="text-xs font-bold uppercase tracking-widest text-red-600">Our Process</span>
            ...
            {process.map(({ step, icon: Icon, title, desc }, i) => (
              <div key={step} className="relative bg-white rounded-2xl border border-gray-100 p-7 flex flex-col items-center text-center shadow-sm">
                <div className="relative mb-5">
                  ...
                  <span className="absolute -top-2 -right-2 w-6 h-6 bg-red-600 rounded-full text-white text-[10px] font-extrabold flex items-center justify-center">
                    {i + 1}
                  </span>
                </div>
        ...
        <div className="container-custom">
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-10 lg:gap-16">
            ...
            <div className="lg:col-span-1">
              <span className="text-xs font-bold uppercase tracking-widest text-red-400">Why Us</span>
            ...
            <div className="lg:col-span-2 grid grid-cols-1 sm:grid-cols-2 gap-4">
              {whyUs.map(({ icon: Icon, title, desc }) => (
                <div key={title} className="flex items-start gap-4 bg-white/5 border border-white/10 rounded-xl p-5">
                  <div className="w-10 h-10 bg-red-600/20 rounded-lg flex items-center justify-center flex-shrink-0 mt-0.5">
        ...
        <div className="container-custom">
          <div className="grid grid-cols-1 lg:grid-cols-5 gap-10 lg:gap-16 items-start">
            ...
            <div className="lg:col-span-2">
              <span className="text-xs font-bold uppercase tracking-widest text-red-600">Cost Estimator</span>
      ...
      <div className="bg-gradient-to-r from-red-600 to-rose-700 py-16">
        <div className="container-custom text-center">
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <Link
              href="/contact"
              className="inline-flex items-center justify-center gap-2 bg-white text-red-600 hover:bg-red-50 font-bold px-8 py-4 rounded-xl transition-colors"
            >
