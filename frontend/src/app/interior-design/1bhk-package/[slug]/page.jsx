...
import { Clock, Shield, CheckCircle, Package } from 'lucide-react';
...
async function fetchPackage(slug) {
  try {
    const res = await fetch(`${API_BASE}/1bhk-packages/slug/${slug}`, { next: { revalidate: 3600 } });
...
export async function generateMetadata({ params }) {
  ...
  if (!pkg) return { title: '1 BHK Package Not Found' };
  const title = pkg.metaData?.title || `${pkg.name} – 1 BHK Interior Package | HomelineTeam`;
  const description = pkg.metaData?.description || pkg.description || '';
  return {
    title,
    description,
    keywords: pkg.metaData?.keywords || ['1bhk interior package', '1bhk design', 'HomelineTeam'],
    alternates: { canonical: `https://www.homelineteam.com/interior-design/1bhk-package/${slug}` },
    openGraph: {
      title: pkg.name,
      description,
      images: pkg.mainImages?.[0] ? [{ url: pkg.mainImages[0] }] : [],
      url: `https://www.homelineteam.com/interior-design/1bhk-package/${slug}`,
...
export default async function OneBHKDetailPage({ params }) {
  const { slug } = await params;
      ...
      {inclusions.length > 0 && (
        <div className="bg-gray-50 rounded-xl p-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-4 gap-y-2">
            ...
            {inclusions.length > 8 && (
              <p className="text-xs text-primary-500 font-medium col-span-2">+{inclusions.length - 8} more items included</p>
            )}
          </div>
        </div>
      )}
    ...
    '@graph': [
      {
        ...
        itemListElement: [
          ...
          { '@type': 'ListItem', position: 4, name: pkg.name, item: `https://www.homelineteam.com/interior-design/1bhk-package/${slug}` },
        ],
      },
      {
        '@type': 'Product',
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="bg-white border-b border-gray-100">
        <div className="container-custom py-3 flex items-center gap-2 text-xs text-gray-500">
          ...
          <span>/</span>
          <Link href="/interior-design/1bhk-package" className="hover:text-primary-600 transition-colors">1 BHK Package</Link>
          <span>/</span>
      ...
      <DesignDetailClient
        product={pkg}
        similar={similar}
        backHref="/interior-design/1bhk-package"
        backLabel="Back to 1 BHK Packages"
