        ...
        return (
          <div key={label} className="flex items-center flex-1 last:flex-none">
            <div className="flex flex-col items-center gap-1">
              ...
              <span className={`text-[10px] font-semibold hidden sm:block ${active ? 'text-red-600' : done ? 'text-green-600' : 'text-gray-400'}`}>
                {label}
              </span>
            </div>
            {i < STEPS.length - 1 && (
...
function LiveEstimate({ estimate }) {
  ...
  return (
    <div className="flex items-center gap-2 bg-red-50 border border-red-100 rounded-xl px-4 py-2.5 mb-6">
      <IndianRupee className="w-4 h-4 text-red-500 flex-shrink-0" />
      <span className="text-xs text-red-600 font-medium">Live estimate:</span>
      <span className="text-sm font-extrabold text-red-700">{fmt(estimate.total)}</span>
      <span className="text-[10px] text-red-400 ml-auto hidden sm:block">updates as you fill the form</span>
    </div>
  );
}
...
export default function InteriorDesignCalculator() {
  ...
  if (done && estimate) {
    ...
    const bars = [
      ...
      { label: 'Materials',         value: breakdown.materials, pct: 40, color: 'bg-red-500'  },
    ...
    if (success) {
      return (
        <div className="bg-white rounded-2xl border border-gray-100 shadow-lg p-8 text-center">
          ...
          <button onClick={reset} className="inline-flex items-center gap-2 bg-red-600 hover:bg-red-700 text-white font-bold px-6 py-3 rounded-xl transition-colors">
        ...
        <div className="p-6 sm:p-8">
          ...
          <div className="bg-red-50 border border-red-100 rounded-xl p-5 mb-6">
            ...
            <input
              ...
              className="w-full px-3 py-2.5 border border-gray-200 rounded-xl text-sm text-gray-800 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-red-500 focus:border-transparent mb-3"
            />

            {error && (
              <p className="text-red-600 text-xs font-medium bg-red-100 px-3 py-2 rounded-xl mb-3">{error}</p>
            )}

            <button
              onClick={handleSubmit}
              disabled={!data.name.trim() || !data.phone.trim() || isSubmitting(formId)}
              className="w-full flex items-center justify-center gap-2 bg-red-600 hover:bg-red-700 disabled:opacity-50 disabled:cursor-not-allowed text-white font-bold py-3 rounded-xl transition-colors"
            >
              {isSubmitting(formId)
                ? 'Sending…'
    ...
    <div className="bg-white rounded-2xl border border-gray-100 shadow-lg p-6 sm:p-8">
          ...
          <div className="grid grid-cols-2 gap-3">
            {HOME_TYPES.map(ht => (
              ...
              >
                ...
                <div className="text-xs font-bold text-red-600 mt-1">Starting {fmt(ht.basePrice)}</div>
              </button>
            ))}
          </div>
        </div>
        ...
        <div className="space-y-6">
          ...
          <div>
            ...
            <input
              ...
              className="w-full px-3 py-2.5 border border-gray-200 rounded-xl text-sm text-gray-800 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-red-500"
            />
          </div>
          ...
          <div className="space-y-3">
            <div className="grid grid-cols-2 gap-3">
              <input
                ...
                onChange={e => set('name', e.target.value)}
                placeholder="Full Name *"
                className="w-full px-3 py-3 border border-gray-200 rounded-xl text-sm text-gray-800 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-red-500"
              />
              <input
                type="tel"
                value={data.phone}
      ...
      <div className="flex gap-3 mt-8">
        ...
        <button
          ...
          className="flex-1 flex items-center justify-center gap-2 bg-red-600 hover:bg-red-700 disabled:opacity-40 disabled:cursor-not-allowed text-white font-bold py-3 rounded-xl transition-colors"
        >
          {step === 4
