...
import { Plus, Edit, Trash2, Layers, X, Package, Eye, ToggleLeft, ToggleRight } from 'lucide-react';
...
const OPENING_OPTIONS = [
  { type: 'sliding',  name: 'Sliding',  description: 'Slides open' },
  { type: 'swinging', name: 'Swinging', description: 'Swings open' },
];

const MATERIAL_OPTIONS = [
  { category: 'carcass',  material: 'Plywood',          quality: 'good'    },
  { category: 'carcass',  material: 'MDF',               quality: 'basic'   },
  { category: 'carcass',  material: 'Particle Board',    quality: 'basic'   },
  { category: 'shutter',  material: 'Laminate',          quality: 'good'    },
  { category: 'shutter',  material: 'Acrylic',           quality: 'better'  },
  { category: 'shutter',  material: 'Glass',             quality: 'premium' },
  { category: 'shutter',  material: 'Membrane',          quality: 'good'    },
  { category: 'finish',   material: 'Matte',             quality: 'good'    },
  { category: 'finish',   material: 'Glossy',            quality: 'better'  },
  { category: 'finish',   material: 'Satin',             quality: 'better'  },
  { category: 'hardware', material: 'Soft Close Hinges', quality: 'better'  },
  { category: 'hardware', material: 'Standard Hinges',   quality: 'basic'   },
  { category: 'hardware', material: 'Channel Runners',   quality: 'good'    },
];
...
const FEATURE_OPTIONS = [
  ...
  { id: 'pull-out',           name: 'Pull-out Trays',      category: 'storage'      },
  { id: 'shoe-rack',          name: 'Shoe Rack',           category: 'storage'      },
  { id: 'tie-belt-holder',    name: 'Tie & Belt Holder',   category: 'organization' },
  { id: 'full-length-mirror', name: 'Full-Length Mirror',  category: 'hardware'     },
];
...
const DOOR_TYPES    = ['2-door', '3-door', '4-door', '5-door', 'sliding'];
...
const EMPTY_FORM = {
  ...
  defaultOpening: null,
  defaultType: null,
  defaultMaterials: [], defaultFeatures: [],
  availableOpenings: [], availableTypes: [],
...
export default function WardrobeProductsPage() {
  ...
  const fetchItems = async () => {
    try {
      ...
      showToast('error', 'Failed to load wardrobe products');
    } finally {
      setLoading(false);
    }
  };
  ...
  const openEdit = (item) => {
    ...
    setForm({
      ...
      wardrobeMetadata: item.wardrobeMetadata || EMPTY_FORM.wardrobeMetadata,
      metaData: {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      fd.append('availableOpenings', JSON.stringify(form.availableOpenings));
      ...
      closeForm();
      fetchItems();
      showToast('success', editing ? 'Wardrobe product updated' : 'Wardrobe product created');
    } catch (e) {
      showToast('error', e?.response?.data?.message || 'Failed to save wardrobe product');
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await apiClient.delete(`/wardrobe-products/${id}`);
      fetchItems();
      showToast('success', 'Wardrobe product deleted');
    } catch {
      showToast('error', 'Failed to delete wardrobe product');
    }
  };

  const set   = (key, val) => setForm(f => ({ ...f, [key]: val }));
  const setWM = (key, val) => setForm(f => ({ ...f, wardrobeMetadata: { ...f.wardrobeMetadata, [key]: val } }));
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-900">Wardrobe Products</h2>
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
        <div className="px-5 py-4 border-b border-gray-100 flex items-center justify-between">
          <div className="flex items-center gap-2.5">
            ...
            <h3 className="font-semibold text-gray-900">Wardrobe Products</h3>
          </div>
        ...
        {loading ? (
          ...
          <div className="py-16 text-center">
            ...
            <p className="text-gray-900 font-semibold mb-1">No wardrobe products yet</p>
            <p className="text-gray-400 text-sm mb-5">Add your first wardrobe product to get started</p>
            <button
              onClick={openCreate}
              ...
              <tbody className="divide-y divide-gray-50">
                {items.map(item => (
                  <tr key={item._id} className="hover:bg-gray-50 transition-colors">
                      ...
                      <div className="flex items-center gap-3">
                        <div className="w-10 h-10 rounded-lg overflow-hidden bg-gray-100 flex-shrink-0">
                          {item.mainImages?.[0]
                            ...
                            : <div className="w-10 h-10 flex items-center justify-center"><Layers className="w-5 h-5 text-gray-400" /></div>
                          }
                        </div>
                    ...
                    <td className="px-5 py-3.5">
                      <span className="text-sm text-gray-700">{item.defaultType || <span className="text-gray-400">—</span>}</span>
                    </td>
                    <td className="px-5 py-3.5">
                      {item.hasVariants
          ...
          <div style={{ position: 'relative', marginLeft: 'auto', width: '100%', maxWidth: '42rem', height: '100%', backgroundColor: 'white', display: 'flex', flexDirection: 'column', boxShadow: '0 25px 50px -12px rgba(0,0,0,0.25)', overflow: 'hidden' }}>
            ...
            <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 flex-shrink-0">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 className="text-base font-bold text-gray-900">{editing ? 'Edit Wardrobe Product' : 'Add Wardrobe Product'}</h2>
                  <p className="text-xs text-gray-400">{editing ? 'Update product details' : 'Create a new wardrobe product'}</p>
                </div>
              </div>
            ...
            {/* Scrollable body */}
            <form id="wardrobe-form" onSubmit={handleSubmit} className="flex-1 overflow-y-auto px-6 py-5 space-y-6">
              {/* Basic Info */}
              <section>
                ...
                <div className="space-y-4">
                  <div className="grid grid-cols-2 gap-4">
                    <Field label="Product Name *">
                      <input
                        ...
                        placeholder="e.g. Modern 3-Door Wardrobe"
                        required
                      />
                    </Field>
                    <Field label="Slug *">
                      <input
                        ...
                        placeholder="modern-3-door-wardrobe"
                        required
                      />
                    </Field>
                  </div>
                  <Field label="Description">
                    <textarea
                      ...
                      placeholder="Describe this wardrobe product..."
                    />
                  </Field>
                  <Field label="Category">
                    <select className={inputCls} value={form.category} onChange={e => set('category', e.target.value)}>
                      <option value="wardrobe">Wardrobe</option>
                      <option value="wardrobe-package">Wardrobe Package</option>
                    </select>
                  </Field>
                </div>
              </section>
              ...
              {/* Opening Type */}
              <section>
                <h3 className="text-sm font-bold text-gray-900 mb-3 pb-2 border-b border-gray-100">Opening Type</h3>
                <div className="grid grid-cols-2 gap-3">
                  {OPENING_OPTIONS.map(opt => (
                    <button
                      key={opt.type}
                      type="button"
                      onClick={() => set('defaultOpening', form.defaultOpening === opt.type ? null : opt.type)}
                      className={`p-3 rounded-xl border text-left text-sm transition-colors ${
                        form.defaultOpening === opt.type
              ...
              {/* Default Type */}
              <section>
                <h3 className="text-sm font-bold text-gray-900 mb-3 pb-2 border-b border-gray-100">Default Type</h3>
                <div className="grid grid-cols-3 gap-3">
                  {TYPE_OPTIONS.map(opt => (
                    <button
                      key={opt.type}
                      type="button"
                      onClick={() => set('defaultType', form.defaultType === opt.type ? null : opt.type)}
                      className={`p-3 rounded-xl border text-left text-sm transition-colors ${
                        form.defaultType === opt.type
                          ? 'border-blue-500 bg-blue-50 text-blue-700'
                          : 'border-gray-200 bg-white text-gray-700 hover:border-blue-300'
                      }`}
                    >
              ...
              <section>
                ...
                {form.hasVariants && (
                  <div className="space-y-3">
                        ...
                        <div className="grid grid-cols-2 gap-3">
                          ...
                          <Field label="Type *">
                            <select
                              className={inputCls}
                              value={v.type || ''}
                              onChange={e => { const u = [...form.variants]; u[i] = { ...u[i], type: e.target.value }; set('variants', u); }}
                            >
                    ...
                    <button
                      type="button"
                      onClick={() => set('variants', [...form.variants, { label: '', type: '', sku: '', price: '', mrp: '', stock: '', isActive: true }])}
              ...
              {/* Wardrobe Specs */}
              <section>
                <h3 className="text-sm font-bold text-gray-900 mb-3 pb-2 border-b border-gray-100">Wardrobe Specifications</h3>
                <div className="space-y-4">
                  <div>
                    <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-2">Suitable For</p>
                    <TagToggle
                      options={SUITABLE_FOR}
                      selected={form.wardrobeMetadata.suitableFor}
                      onToggle={opt => setWM('suitableFor', toggleArr(form.wardrobeMetadata.suitableFor, opt))}
                      upper
                    />
                  </div>
                  <div>
                    ...
                    <TagToggle
                      ...
                      selected={form.wardrobeMetadata.style}
                      onToggle={opt => setWM('style', toggleArr(form.wardrobeMetadata.style, opt))}
                    />
                  </div>
                  <div>
                    ...
                    <TagToggle
                      ...
                      selected={form.wardrobeMetadata.colorScheme}
                      onToggle={opt => setWM('colorScheme', toggleArr(form.wardrobeMetadata.colorScheme, opt))}
                    />
                  </div>
                  <div className="grid grid-cols-2 gap-4">
                    <Field label="Delivery Time">
                      <input className={inputCls} value={form.wardrobeMetadata.deliveryTime} onChange={e => setWM('deliveryTime', e.target.value)} placeholder="e.g. 15-20 days" />
                    </Field>
                    <Field label="Warranty">
                      <input className={inputCls} value={form.wardrobeMetadata.warranty} onChange={e => setWM('warranty', e.target.value)} placeholder="e.g. 2 years" />
                    </Field>
                  </div>
                </div>
              </section>
              ...
              <section>
                ...
                <Field label="Tags (comma separated)">
                  <input className={inputCls} value={form.tags} onChange={e => set('tags', e.target.value)} placeholder="wardrobe, modern, premium" />
                </Field>
              </section>
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-6">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="border border-gray-100 rounded-xl p-4">
                  <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-1">Default Type</p>
                  <p className="text-sm font-semibold text-gray-900">{viewItem.defaultType || '—'}</p>
                </div>
                <div className="border border-gray-100 rounded-xl p-4">
                  <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-1">Opening</p>
                  <p className="text-sm font-semibold text-gray-900 capitalize">{viewItem.defaultOpening || '—'}</p>
                </div>
                <div className="border border-gray-100 rounded-xl p-4">
                  <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-1">Materials · Features</p>
                  <p className="text-sm font-semibold text-gray-900">
                    {viewItem.defaultMaterials?.length || 0} · {viewItem.defaultFeatures?.length || 0}
                  </p>
                </div>
              </div>
                    ...
                    {viewItem.variants.map((v, i) => (
                      <div key={i} className="flex items-center justify-between border border-gray-100 rounded-xl px-4 py-3">
                        <div>
                          ...
                          <p className="text-xs text-gray-400">{v.type}{v.sku ? ` · SKU: ${v.sku}` : ''}</p>
                        </div>
              ...
              {/* Wardrobe specs */}
              {viewItem.wardrobeMetadata && (
                <div>
                  <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-3">Wardrobe Specifications</p>
                  <div className="grid grid-cols-2 gap-3">
                    {viewItem.wardrobeMetadata.suitableFor?.length > 0 && (
                      <div className="border border-gray-100 rounded-xl p-3">
                        <p className="text-xs text-gray-500 mb-1.5">Suitable For</p>
                        <div className="flex flex-wrap gap-1">
                          {viewItem.wardrobeMetadata.suitableFor.map(s => (
                            <span key={s} className="px-2 py-0.5 bg-blue-100 text-blue-700 text-xs font-semibold rounded-full">{s.toUpperCase()}</span>
                          ))}
                        </div>
                      </div>
                    )}
                    {viewItem.wardrobeMetadata.style?.length > 0 && (
                      <div className="border border-gray-100 rounded-xl p-3">
                        <p className="text-xs text-gray-500 mb-1.5">Style</p>
                        <div className="flex flex-wrap gap-1">
                          {viewItem.wardrobeMetadata.style.map(s => (
                            <span key={s} className="px-2 py-0.5 bg-violet-100 text-violet-700 text-xs font-semibold rounded-full capitalize">{s}</span>
                          ))}
                        </div>
                      </div>
                    )}
                    {viewItem.wardrobeMetadata.deliveryTime && (
                      <div className="border border-gray-100 rounded-xl p-3">
                        <p className="text-xs text-gray-500 mb-0.5">Delivery Time</p>
                        <p className="text-sm font-semibold text-gray-900">{viewItem.wardrobeMetadata.deliveryTime}</p>
                      </div>
                    )}
                    {viewItem.wardrobeMetadata.warranty && (
                      <div className="border border-gray-100 rounded-xl p-3">
                        <p className="text-xs text-gray-500 mb-0.5">Warranty</p>
                        <p className="text-sm font-semibold text-gray-900">{viewItem.wardrobeMetadata.warranty}</p>
                      </div>
                    )}
                  </div>
                </div>
